...
postdir = check_path(rootdir,'post_data')
savefigs=False
...
def arg_parser(): 
    ...
    parser.add_argument('--save_eval', type=bool, default=True)
...
if __name__ == '__main__':
    ...
    n = -2 # checkpoint index
    modeltype = args.modeltype # '3d'
    if modeltype=='shotgun': 
        ...
        filename =  os.path.join(os.path.expanduser('~/Research/Github/'),'PyTorch-VAE','configs/WC_vae3dmp2.yaml')
        ckpt_path = glob.glob(os.path.expanduser('~/Research/FMEphys/logs2/VAE3dmp/version_1/checkpoints/*.ckpt'))[n]
    else:
        raise ValueError(f'{modeltype} is not a valid model type')
    print(ckpt_path)
    ...
    ##### Adjust paths for config based on computer #####
    if modeltype=='shotgun': 
    ...
    latInterp = interp1d(worldT,zt,axis=0, bounds_error = False)
    ...
    ##### Plot STA #####
    fig, axs = plt.subplots(1,figsize=(5,25))
    ...
    if args.savefigs:
        fig.savefig(os.path.join(FigurePath,'STA_Model{}.png'.format(config['exp_params']['imgH_size'])))
